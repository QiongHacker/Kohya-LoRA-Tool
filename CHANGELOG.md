...
- **背景**（2026-09-16 用户反馈）：他的数据集有大量**背面 / 侧身图**（背面、右侧半身、
  右侧全身、左侧全身、左侧上半、回头…），而 WD14 这类打标器**在看不到正面的视角上会直接漏标**
  → 角色特征 `twintails`（双马尾）只有 20/21（95%）✗
- **旧告警说的是**：「人物一致性不足，建议统一训练集特征或补齐图片后再训」✗
  —— 「一致性不足」**是错的**（数据集一致性没问题）；
  「统一训练集特征」**更没用**（修不好一个"打标器看不到"的东西）。
  **它会把用户引去改数据集、甚至删掉侧身图** ✗
- **已改**：如实说明「常见原因不是你的数据集不一致，而是**视角遮挡让自动打标漏标**」，
  并给出可执行的下一步 —— 打开「标签统计」→ 选中它 → 点「★ 锁进固定前缀」✓
- 标签统计窗的图例也补了同一句（看到 `★` 时不会误以为是自己数据集有问题）✓
- **回归测试**：`HIGH_COVERAGE_TAG_LOCK_OK` 里加了正向断言
  （用正向而非负向 —— 代码注释会引用旧文案做历史说明，负向断言会误报）。

### 新增：标签统计 —— 高覆盖率特征可「锁进固定前缀」

- **用户场景**（2026-09-16）：某角色的关键特征覆盖率是 `green hair` 20/21（95%）、
  `witch hat` 17/21（81%）—— **全都够不着强绑定的 100% 门槛**，
  于是自动锁定只锁到了 `long hair` 这种没辨识度的词 →
  **单写触发词唤不出角色**，用户因此怀疑"强绑定不生效"。
  （他自己实测：手动补上 `green hair` 就"非常像"了 ✓ 说明这些 81~95% 的特征才是该锁的。）
- **新增**：
  - 标签统计里显示**覆盖率**：`★ 4/5（80%）· 未锁定`（只标 ≥80% 且未达 100% 的），
    并加一行说明「强绑定只锁 100% 的词」；
  - **「★ 锁进固定前缀」按钮**：选中后一键把这批特征锁进固定前缀。
- **实现走 `|||` 手动固定区**（新增 `lock_tags_to_prefix()`），
  **复用已有且已有回归测试的机制**：预处理时它会被 `apply_strong_binding` 变成真正的
  固定前缀、`keep_tokens` 设为前缀长度、`|||` 自动删掉 ——
  界面不必自己决定"什么该锁、keep_tokens 算几"。
  而且它**写在标签里**：看得见、可撤销（接入既有的「↩ 撤销上次修改」），
  不像"全局覆盖率阈值"那样会在用户不知情时改变行为。
- ⚠️ 确认框必须说清代价：**锁进去的特征出图时会固定出现**（锁了帽子就再也脱不掉），
  且**要重跑一次预处理**才生效。
- ⚠️ **强绑定不再静默**：自动模式下如果一个可锁特征都没有（固定前缀只剩 trigger），
  以前这里**一个字都不打** ✗ → 现在明确告知「强绑定等于没生效」+
  列出覆盖率最高的一批 + 两条出路（统一训练集特征 / 用上面的按钮手动锁定）。
- **回归测试**：`MANUAL_LOCK_TAGS_TO_PREFIX_OK`（写入 → `apply_strong_binding` 消费 →
  `keep_tokens` 的**完整往返** —— 只验"写进去了"是不够的）、`HIGH_COVERAGE_TAG_LOCK_OK`。
- 实测方式：**真实构造标签统计窗**端到端跑一遍 —— 覆盖率行、按钮可见性、确认框文案、
  写入结果 `qiansui321, green hair ||| long hair, witch hat, 1girl`、撤销快照，逐项确认。
- ⚠️ 踩坑记录：触发词给定时**不要**依赖"从主窗口表单取"（编辑器可能是为另一个项目打开的、
  或表单当时为空）—— 取不到时应**保留原本排在最前的那个标签**作为固定区领头；
  即便判错也无害，因为 `apply_strong_binding` 的手动模式会自己把 trigger 补到固定区最前。

> 来源：用户 qiansui 的一次真实故障排查（打标崩溃 + 训练崩溃 + 安装卡住）。
> 主线很一致：**工具的判断基本是对的，但它说的话让用户没法据此行动** ——
> 从「退出码 1」到「缺 onnxruntime」到「下载失败」到写死的「Python 3.12」徽章，
> 全是这个毛病。用户最后去问了另一个 AI，在 cpu/gpu 变体之间折腾几小时后，
> **回到了工具原本就给的那条路**。

### 修复★：下载/安装任务可能永久卡住（「活干完了，但任务不结束」）

- **现象**（用户原话）：「下载训练内核的时候不知道为什么老是在最后卡住……他好像下完了
  但是没有自动结束任务，然后我手动结束任务，再重新点一次那个下载内核，他又说安装好了。」
- **根因**：`run_stream` 用 `for line in proc.stdout` 阻塞读，等的是**管道 EOF**，
  而不是**进程退出** ✗ Windows 上子进程的 stdout 管道句柄会被**孙进程**继承
  （`git clone` 的 `git-remote-https`、pip 的构建子进程、杀软扫描进程等）——
  只要还有孙进程握着它，管道就永不 EOF → **永久阻塞** ✗
  「重启后检测又说安装好了」正是铁证：**活早就干完了，卡的是收尾**。
- **已修**：读取移到 daemon 线程，主循环以 `proc.poll()`（进程退出）为判据；
  进程退出后再额外排空 2 秒收缓冲尾巴，但**绝不为管道无限等待**。
- ⚠️ **踩坑记录**：第一版改完**仍然卡 30 秒** —— 因为收尾处的 `proc.stdout.close()`
  会等读取线程释放 io 内部锁（实测自身阻塞 **29.92 秒**），等于把挂死从循环搬到了收尾处。
  **那个管道不能 close**：daemon 线程会在管道真正 EOF 时自己读完退出。
- **影响面**：所有走 `run_stream` 的任务（安装 / 训练 / 预处理）。

### 修复：WD14 打标的「自愈」从未生效（判据问错了问题）

- **根因**：`_has_wd14_deps` 的注释写「检查**能否 import**」，实现却用
  `importlib.util.find_spec`（只查**包在不在**）✗ 而 onnxruntime 最要命的故障形态
  恰恰是**「找得到、import 就死」**（DLL 级故障，零输出、无 traceback）
  → 被判为「已就绪」→ `_ensure_onnx` 的自动补装**永远不触发** ✗
- **连带误导**：日志说「**缺** torch/onnxruntime」，真实状态却是「装了但导入就崩」——
  「缺」把用户引向"再装一个"，而装错变体（gpu / directml）反而让冲突更严重 ✗
- **已修**：
  1. 判据改为**真实 import 探测**（子进程，区分 能用 / 崩溃 / 没装 / 其它错误）；
  2. 「装了但导入就崩」走**清干净重装**：卸载**全部三个变体**
     （onnxruntime / -gpu / -directml，它们装进同一个模块目录）→ 删残留目录 → 只装 CPU 版；
  3. 崩溃后**自动修复并重试一次**打标（以前只打印一句要用户手打的 pip 命令）；
  4. 修复清单补齐：**不要装 gpu 变体** / VC++ 运行库 / 重建环境，并给出
     **带退出码的验证命令**（`print('A') → import → print('B')` + `%ERRORLEVEL%`）——
     让「没输出」不再是谜。

### 修复：建训练环境不校验 Python 来源（Anaconda 埋雷）

- **根因**：`install_python` 只判版本范围（`PY_MIN <= ver < PY_MAX`），**不看来源** ✗
  Anaconda 的 3.11.7 落在范围内 → 直接被采用 → 用它建了训练环境
  （而工具自带的官方 Python 3.12 安装包因此**永远不会被用上**）。
- **后果链**（用户实测）：conda 自带的 native DLL 污染 → 原生库 0xC0000005；
  用户按建议「屏蔽 Anaconda」→ venv 的基座消失 → 「训练内核损坏、找不到 python 路径」✗
- **已修**：新增 `_python_is_conda()`（路径关键字 + `conda-meta` 双重判定）；
  命中时**优先安装内置官方 Python**（与 conda 完全隔离）并说明两个后果。
  三条官方途径都失败时仍会用 conda 的，但**明确告知风险**（不硬阻断，避免误伤）。

### 优化：状态徽章如实显示实际 Python 版本与来源

- `● Python 3.12` 是**写死的** ✗ 手里是 3.11 甚至 Anaconda 也照样显示 3.12
  → 用户排查时会误以为自己环境符合要求 ✗
- 现在显示**实际版本**，来自 Anaconda 时标红警示：`● Python 3.11.7（Anaconda ⚠）`

### 新增：训练侧 native 崩溃诊断（以前只报「退出码 1」）

- **现象**：`anima_train_network.py` 原生崩溃（0xC0000005）、**Python 层零输出**；
  accelerate 把它转成退出码 1 → 工具只报「训练结束，退出码 1，请查看上方日志」✗
  **`3221225477` 在全代码库 0 命中** —— 工具根本不认识这个错误码。
- **已修**：新增 `_diagnose_native_crash()`，**从日志文本**识别（不能只看退出码：
  这里拿到的是 accelerate 的 1，真实码被吞在 traceback 里），给出排查步骤
  （官方 Python / VC++ 运行库 / 重建环境），并明确「**重复训练不会变好**」。
  它接在 `_diagnose_optimizer_failure` 这个**公共入口**上 → 六条训练路径
  （SD/SDXL、Anima、Krea2、FLUX.2、视频、AT 图像）全部覆盖。

### 文案：自动换源不再说「失败」，且必须有结论

- Anima 文本编码器下载：`魔搭下载失败（…（文件缺失或命名异常）），自动改用…`
  → 读起来像绕口令，而紧接着的 hf-mirror 其实**成功了** ✗
  用户原话：「也不知道到底成功还是失败，下载倒是有进行」。
- 现在：换源与原因**分两行**，并给出明确结论 `[Anima] ✓ 文本编码器已就绪：…`；
  三处镜像切换提示统一改为「当前镜像没下成，切换备用镜像重试…」。
- **定个通则**：凡是「自动换源 / 自动重试」的分支，不要在中间用「失败」二字 —— 结果还没定。

### 测试

- `engine_install_smoke_test.py`：`RUN_STREAM_NO_PIPE_HANG_OK`（真实复现「子进程退出、
  孙进程仍持有管道」，旧实现 30.08s → 现 2.26s）、`WD14_DEP_PROBE_REAL_IMPORT_OK`、
  `WD14_ONNX_CLEAN_REINSTALL_OK`
- `smoke_test.py`：`PYTHON_ENV_SOURCE_GUARD_OK`、`NATIVE_CRASH_DIAGNOSIS_OK`
- ⚠️ **踩坑记录**：本想用「假模块 + PYTHONPATH」造真实崩溃场景，但实测**本机 PYTHONPATH
  传不进子进程**（子进程 `sys.path` 里根本没有它）—— 代码库早就写明「不依赖 PYTHONPATH
  传递，环境变量在某些宿主里不保证生效」。已改为**纯函数分类**断言（照 `_classify_ort_probe`
  的现成范式），确定性更好。

### 新增：Anima 的文本编码器 / VAE 可「指定已有文件」

- **背景**（2026-09-16 用户反馈）：他本机已经在 **ComfyUI 目录**里放了这些模型，但工具只在
  3 个固定 APPDATA 目录里、按**精确目录名**找（`Qwen3-0.6B` / `Anima_vae`），
  找不到就直接下载（Qwen3 1.2GB + VAE 0.3GB，国内约 1.3MB/s ≈ **20 分钟**）。
  日志实证确实走了下载（`[Anima] 从魔搭下载 Qwen3-0.6B/…`）。
  **用户原话：「一个盘里放了两样的模型 / 占两份空间」** —— 既白等，又多占一份磁盘。
- **已加**：底模下拉「没有模型？点这里下载」里的 Anima 分支**换成真对话框**
  （messagebox 放不下按钮），里面：
  - 显示两个组件**当前用哪个**，并区分「你指定」/「自动检测」；
    没有则明确写「⚠ 未找到 —— 训练时会自动下载」；
  - 「📁 选文件夹 / 📄 选文件」指定 Qwen3 文本编码器，「📄 选文件」指定 Qwen-Image VAE；
  - 指定后**存进设置**，训练时优先使用、**不再下载**。
- **指向 ComfyUI 那份是安全的**（已核对）：训练只**读取**指定路径，工具自己的目录创建
  （`os.makedirs`）只作用于 APPDATA 下的 `KohyaLoraTool\anima`，
  **不会往你的 ComfyUI 目录里写任何东西、也不会复制一份**。
- **★ 训练前会先问一句，不再默默下载**：组件对话框原先只挂在
  「没有模型？点这里下载」里 —— 而**已有底模的用户根本不会点那个按钮** ✗，
  等于入口形同虚设。现在改为：Anima 训练开始前发现组件缺失时**先弹一句**
  「要现在指定已有的文件吗（用你的文件、不下载、不多占空间）/ 否 = 自动下载（约 20 分钟）」。
  - **两个都齐时静默通过**，绝不打扰已就绪的用户；缺任一个就提示
    （VAE 那 0.3GB 一样是几分钟 + 多占一份）。
  - 选「是」打开的对话框会**阻塞到用户选完**（`wait=True` → `wait_window`）——
    少这一句的话用户还没选，训练就已经开始下载了。
  - 实测：`wait=True` 真实阻塞 **1.57 秒**（定时关闭验证），阻塞行为确认有效。
- **校验前置**：选的时候就说清为什么不能用 —— 非标准权重名（如
  `model.safetensors (1).safetensors`，transformers 不认）、文件夹里没有权重、
  safetensors 头尾不一致、VAE 选了文件夹 —— 而不是等训练时才炸 ✗
- **路径失效自动回落**：指定的文件被挪走/删掉时当作未设置、回落到自动检测 ——
  不会因为记着一个旧路径就让训练卡死。
- **顺带修复**：VAE 的完整性校验此前**无条件**按 safetensors 格式解析，
  而目录扫描与选择入口都接受 `.pth` ✗ → 合法的 `.pth` 会被误报成「文件损坏/不完整」。
  现已限定只对 `.safetensors` 做该校验。
- **回归测试**：`ANIMA_COMPONENT_PICKER_OK`。
- 实测方式：核心链路（校验拒绝/接受 → 查找命中 → **`_ensure_anima_components()`
  0.00 秒直接返回指定文件、不触发下载**）+ **真实构造对话框**（按钮、状态文案、
  指定后刷新为「✓ 当前使用（你指定）」）。
...
### 修复★：v-prediction 底模（如 NoobAI-XL vPred）采样预览全是噪点

- **用户现象**：底模 `noobaiXLNAIXL_vPred10Version.safetensors`，采样预览 200/400/600 步**全是彩色噪点**。
- **根因**：v-pred 与 eps 的**模型结构完全相同**（差的只是训练目标），所以
  `detect_base_type` 靠 safetensors 键分类架构**认不出来**，工具此前也**从未传过
  `--v_parameterization`**（全代码库 0 命中）→ sd-scripts 按 **epsilon** 训练并采样。
- **最迷惑人的地方**：训练 loss 看着完全正常（该用户 `avr_loss=0.0965`），
  所以很容易被误判成「LoRA 没练好」而一直调参数。实际上**每一步都会是噪点**。
- **已修**：新增 `_looks_like_vpred()`（按文件名识别 `vPred` / `v-pred` / `v_pred`），
  命中时自动加 `--v_parameterization` 并打印一行日志。
- **顺带修**：第一引擎的采样提示词此前**没写分辨率**（只有 musubi 引擎写了 `--w/--h`），
  1024 训练的 SDXL 会按采样器默认 512 出图 → 图小且糊。现已按训练分辨率写入。
- **回归测试**：`VPRED_PARAMETERIZATION_OK`（识别函数不能误伤 eps 版 / 普通模型 + 接线断言）。
- ⚠️ **对已训练 LoRA 的影响**：本次修复只对**之后的训练**生效。用 v-pred 底模且未加该参数
  训练出来的权重**需要重训**；推理端使用 v-pred LoRA 时也必须配 v-pred 采样。

### 修复：多选模式打开后仍要按 Ctrl（等于没生效）

- **用户反馈**（2026-09-16）：「多选按钮改成和按住 Ctrl 一样的功能，也保留滑动选择能力」。
- **根因**：上一版只把 `selectmode` 从 `browse` 换成 `extended`，而 **`extended` 本来就要求按 Ctrl**
  —— 所以「多选模式」打开后仍然得按 Ctrl，开关等于无效。当时又按要求去掉了说明文字，
  于是这个"没生效"一直没人发现。
...
- **修法**：新增 `_attach_multiselect()` **自己接管鼠标事件**（两个诉求原生无法兼得）：
  按下记录锚点 + 按下前的选中集 → 拖动 = 选中集 ∪ [锚点..当前行]（滑动选择）→
  未拖动 = 切换该行（等价 Ctrl+单击）。图片列表与标签统计窗都挂上。
- **验证**：真实窗口 + 合成鼠标事件逐场景实测 7 项全过（连点累加 / 再点取消 / 拖拽连选 /
  拖拽叠加在已有选中上 / 拖拽后仍能单击切换 / 单选模式仍走原生）。
  ⚠️ 踩坑记录：**合成事件会跨场景污染**（表现为"拖拽之后第一次点击被吞"），
  必须**每个场景独立建窗口**才测得出真实行为 —— 否则会去"修"一个不存在的 bug。
- **回归测试**：`MULTISELECT_CLICK_TOGGLE_OK`。

### 新增：主页「📂 输出目录」按钮

- **背景**（2026-09-16 用户反馈）：想看训练出来的 LoRA，**必须先打开某个项目**
  （只有工作区工具条才有「打开输出文件夹」），或者自己去 `%APPDATA%\KohyaLoraTool\output` 里翻。
- **已加**：主页头部新增「📂 输出目录」，紧挨「💾 数据目录」（同类操作聚一起）。
  视觉顺序：`🧰 小工具 | 🔄 检查更新 | 📂 输出目录 | 💾 数据目录 | ⏳ 训练队列 | ➕ 新建项目`
- **打开的是「输出根目录」而不是某个项目** —— 一级子目录 = 一个项目，一次看到全部，
  不用先猜是哪个项目（这才真正免除「先点开之前的项目」）。工作区那个入口语义不变。
- **顺带修**：原 `cmd_open_output()` 是**裸 `os.startfile`，既无 try/except 也不建目录** ——
  从没训练过时目录不存在，会静默失手。现在统一走 `_open_dir_or_warn()`：
  先 `makedirs(exist_ok=True)`，失败则弹窗给出完整路径（被安全软件拦截时也能看懂）。
- **回归测试**：`HOME_OUTPUT_BUTTON_OK`。
- 实测方式：**真实构造主窗口**（非源码断言）—— 按钮已映射、文案与绑定正确、
  与数据目录同容器同排列；没开项目 → 打开 `<数据根>/output`；开了项目 →
  打开 `<数据根>/output/<项目>`；startfile 抛错时弹提示且不崩。

### 新增：预处理进度条 —— 解析 WD14 打标进度，消灭「是不是卡住了」

- **背景**（2026-09-16 用户反馈）：预处理期间界面没有任何进度反馈，
  WD14 打标又是整段流程里最长的等待，总让人怀疑是不是卡住了。
- **为什么只能解析日志**：预处理跑在**独立子进程**里（`preprocess()` 用 `run_stream`
  拉起 `preprocess.py`），父进程拿不到任何回调 —— 子进程 stdout 是唯一的信息通道。
  所以新增 `core.PreprocessMonitor`（与 `TrainMonitor` 同一套路：线程安全 + `snapshot()`
  轮询），把日志里的 `[WD14] 内置打标进度：done/total` 抽出来交给界面。
- **界面**：预处理期间复用训练监控那块面板（标题临时改为「📊 预处理进度」），
  显示 `内置打标 120 / 450（27%）`；结束后撤掉监控、标题拨回训练语义。
- **两处调用点都接**：单独「数据预处理」**与**「一键开始训练」里的自动预处理 ——
  只接一处的话，「一键」反而比单独预处理更让人干等。
- ⚠️ **两个实测口径（决定了显示方式，别照直觉改）**：
  1. `done` 是**每 5 张**报一次（`preprocess.py` 的 `done % 5 == 0 or done == total`），
     两次更新之间隔个位数秒是**正常**的 → 判定「卡住」的阈值放宽到 3 分钟，
     只有**已知 N/M 却 3 分钟无新进展**才提示（30 秒内的停顿不误报）。
  2. 官方打标脚本（kohya 自带 sd-scripts）走 **tqdm**，它是 `\r` 原地刷新、不是整行输出，
     **拿不到 N/M** → 这条路径下只报「WD14 打标 进行中（已 N 分 N 秒）」，
     **绝不假装 0% 的确定进度**（假的确定条比没有进度更让人以为它死了）。
- **防误报**：只有 `[WD14]` 标签的行才驱动该进度 —— 训练步数、下载字节、
  缓存 tqdm 等其它阶段的 `N/M` 都不会被误当成预处理进度（否则进度条会乱跳）。
- **回归测试**：`PREPROCESS_PROGRESS_OK`（含上述三条边界 + 两处调用点接线断言）。

---

## v0.16.11（2026-09-15）

> 来源：用户「馨」的一组界面反馈（标签编辑器 / 标签统计窗），
> 以及另几位用户报告的「点开始训练后卡住无输出」「删项目后磁盘不释放」「续训承诺不实」等问题。

### 安全：标签批量操作补上「预演 + 撤销」两道闸

- **背景**：用户反馈「有时候多选标签，忘记按 Ctrl 了，导致前面选择删除的标签没了」。
  排查后确认：真正的危险不在选择方式，而在**批量删除/替换直接覆写全部 `.txt`，
  既无确认、也无撤销**（`_do_remove` / `_do_replace` / 统计窗删除三条链路全部如此）。
- **预演（dry-run）**：`batch_remove_tags` / `batch_replace_tags` 新增 `dry_run=True`
  —— 只统计「将影响多少张图 / 移除多少个标签」，**绝不写盘**。
  界面据此先弹确认（`将从【全部 N 张图】中移除这些标签…`），没有再"点一下就直接改完"。
  无匹配时直接提示「未改动任何文件」。
- **撤销**：新增 `restore_captions()` + 每次批量操作前的**原文快照**（只登记真正会变动的文件）。
  工具条新增「↩ 撤销上次修改」，覆盖批量删除 / 批量替换 / 统计窗删除。
- ⚠️ **字节级还原**：`_read_caption_raw()` 必须用 `newline=""` 读 —— 默认的通用换行处理会把
  CRLF 归一成 LF，撤销时就把整个数据集的行尾悄悄改掉（而 `save_caption` 在 Windows 上写的正是 CRLF）。
  回归测试专门覆盖了这条（含「无换行 / 尾随空格 / CRLF」三种原始形态）。

### 新增：图片列表与标签统计窗的「多选模式」开关

- 用户不必再记「要按 Ctrl」：勾上后**点一下即切换选中**，且开关**常驻可见**
  （让模式可见，比让用户记住快捷键可靠）。
- 图片列表默认**单选**（不惊扰老用户），偏好写入 settings.json（`label_editor_multi`），
  两处开关共享同一偏好。
- 实现沿用仓库既有范式（`gui/queue_window.py` 的 `selectmode` 切换）。

### 优化：标签编辑器缩略图自适应

- **原状**：预览图**硬编码 220×140**（`kohya_gui.py` 的 `_load_preview`），
  窗口拉大它纹丝不动 —— 用户反馈「缩略图有点小，明明下面有不少空间」。
- **现在**：窗口尺寸变化时按可用空间重算（默认 420×280，带去抖避免拖动时反复解码重绘）。

> ⚠️ **这一版在这里踩了一串坑，三条约束都写进防回归断言了（`smoke_test.py`），别改回去：**
>
> 1. **绑定必须 `add="+"`** —— `CTkFrame` 内部用 `<Configure>` 维护自己的 canvas/圆角，
>    直接 `bind` 会把它顶掉。（CTk 控件一律如此。）
> 2. **回调里绝不许量其它控件的高度** —— 会成环：
>    缩略图高度 → `body` 的请求高度 → 底部工具条能否分到 pack 空间 → 工具条高度
>    → 预留量 → 缩略图高度…… 实测 **40 轮 `update()` 触发回调 566 次**（死循环），
>    布局整体崩坏、**底部批量工具条被挤成 unmapped** —— 用户报的现象就是
>    「标签编辑器功能组件全没了」。**唯一允许的输入是窗口自身尺寸。**
> 3. **缩略图容器必须是原生 `tk.Frame` + `pack_propagate(False)`** ——
>    `CTkFrame` 是复合控件，`pack_propagate(False)` 转发不到内部 canvas，
>    实测设了 `1236x700` 仍被撑到 `1248x1017`。
>
> 另：预留高度**不能写死物理像素** —— `winfo_*` 返回**物理像素**，窗口 `geometry` 用
> **逻辑像素**（本机 150% 缩放，`1140x780` 实际 `1710x1170`）。本机实测「除缩略图外必须
> 占用」= 顶栏 42 + 工具条 177 + 右侧固定件 591 + 间距 ≈ **848 物理像素**，故预留取
> 590 逻辑像素、经 `ScalingTracker` 折算。
>
> 顺带把 `minsize` 高度 **640 → 680**：本版在标签框下新增了「中文对照」行，右侧面板固定
> 高度变大，640 时最小窗口会把工具条压缩到 ~84%（第二行按钮有被裁风险，实测）。

### 修复：训练前的确认弹窗不写日志 —— 表现为「卡住、没反应、也没报错」

- **用户现象**（2026-09-15，RTX 4090 用户）：导出日志停在 `[OK] 可用图片 20 张` 之后再无任何输出。
- **根因**：「一键开始训练」在预处理成功后会连弹**最多 8 个确认框**（CPU 版 PyTorch 是否重装
  cu128 / 显存不足 / 是否续训 / Anima 合并包 / 参数确认 等），而它们**一行日志都不写**，
  且可能藏在主窗口后面。弹窗阻塞主线程，于是用户既看不到输出也看不到报错，只能判断成卡死。
  该用户命中的是 `_ask_fix_cpu_torch` —— 因其训练环境是 **CPU 版 PyTorch**
  （Windows 上 `pip install torch` 默认装的就是 CPU wheel，故 `detect_torch_backend` 返回 `cpu`；
  注意它只在 torch 能 import 但 `cuda.is_available()==False` 时返回 `cpu`，读不到时返回 `None`，**不是误报**）。
- **修法**：新增统一入口 `App._modal(kind, title, msg, log)` —— **先把日志写出去**再弹窗，
  并把主窗口 `deiconify + lift + focus_force` 提到最前。日志统一以
  `[确认] 弹窗等待你操作：…` 开头，导出日志一眼可见「不是卡死，是在等我点确认」。
  8 处确认框（`_warn_no_nvidia` / `_warn_low_vram` / `_ask_resume` / `_anima_merged_ok` /
  `_confirm_training` / `_ask_fix_cpu_torch` 等）全部改走此入口。
- **回归测试**：`MODAL_DIALOGS_LOGGED_OK` —— 断言「先写日志再弹窗」的顺序，
  以及训练流程里不得再出现裸 `messagebox`（少改一处就会重现同一个投诉）。

### 新增：编辑标签时显示中文对照

- 标签文本框下方新增**只读**中文对照行（词典 17 万条，`to_zh`）。
- **只显示词典里确实有的词条**；整段无命中就整行隐藏 —— 自然语言 caption（Krea2 / FLUX.2 的长句）
  拆出来只有一段且查不到词条，会自动不显示，不会甩一句半吊子机翻误导用户。
- ✅ **`.txt` 里永远只有英文标签**，中文纯展示、不参与保存。

### 测试

- `smoke_test.py` 新增 `标签批量操作：预演 + 快照撤销`（覆盖 dry_run 不写盘、快照原文、
  字节级撤销、替换同样可撤销、CRLF 防回归，以及 GUI 接线断言）。
- 注：`list_dataset_images` **以图片为驱动**（只遍历图片再配对同名 `.txt`）——
  测试里只有 `.txt` 没有配对图片的文件根本不会被列出，这点已写进测试注释。

### 修复：预处理打标崩溃拖垮整轮 / 续训承诺与事实不符 / 打标失败无诊断

> 这批改动在提交 `c01b7f3`（当时补充了完整提交信息，但漏了 CHANGELOG），本版一并纳入。

- **预处理打标 native 崩溃不再拖垮整轮**：用户报「18 张图已处理好，却整批判失败」。
  - 根因：子进程 stdout 被管道接管后是**块缓冲**，硬崩时缓冲区整块丢失，日志一个字都收不到
    （`kohya_core/utils.py` 的 `build_env` 加 `PYTHONUNBUFFERED=1`）。
  - 打标隔离到子进程（`preprocess.py` 的 `_run_wd14_onnx_isolated`）：崩了只丢标签、走兜底
    caption，不再丢已处理好的图片；且不依赖 `PYTHONPATH` 传递。
  - 新增 `_probe_onnxruntime_import` / `_classify_ort_probe`：把「native 崩溃 / 未安装 /
    加载失败」分开并给出可执行修复步骤。
  - 「图片已存在则跳过」的汇总不再放在文件末尾（崩了就永远看不到），改在打标阶段之前打印。
- **续训承诺与事实不符**（用户：被保证可续训，去找却没有，很着急）：
  新增 `_latest_resume_state` 作为**唯一口径**（`_ask_resume` 与停止提示共用，此前两套逻辑）；
  停止时先查快照——有则报快照名，无则明说「本次没有产生可续训的快照」；
  新增 `_steps_per_state` 给出「再跑 N 步到存档点就能续训」。
  根因是引擎只在 epoch 边界写快照，提前停止可能一个都没有，而旧文案无条件承诺「快照已保留」。
...
- **根因**：档位判定用的是 `vram_gb <= 16` 这个硬边界。16G 卡经 DXGI / nvidia-smi 检测常报 15.6~15.9，
  也可能报 16.0x；一旦报成 16.01~16.5，就会掉进 `16 < v < 20` 的「死亡区」——
  拿到 `qfloat8 + 1024 + 完全没有分层交换` 的组合，**必然 OOM**；
  而报成 15.9 的卡则走完整的 16G 快档。同一型号显卡落在阈值两侧表现天差地别，看起来就像「看运气」。
- **修复**：
  - 档位改为按**取整后的 GB** 判定（与 musubi / Fizgig 一致）：`≤19G` 视为低显存档 → `qint8 + 512 + 分层交换`；
  - 17~19G 补上分层交换 0.4（原先这一档一条都没有）；
  - 20G 及以上**保持原样**（不把 1024 降到 512，避免影响本来能跑的机器）；
  - 16G 档取值（qint8 / 512 / 0.3 / rank32）**完全未改**。

### 优化：启动阶段 OOM 的自动重试更有效
- 改为**逐步加大分层交换**（第 2 次 0.4、第 3 次 0.5），不再用同一份配置重跑 3 次；
- 兜底不再要求 `vram_gb <= 16` → 16.01~23.9GB 的卡也能享受（原先这些卡连兜底都拿不到）。

### 优化：启动 OOM 判定收窄
- 只认 `out of memory` / `OutOfMemoryError` / `cudaErrorMemoryAllocation`，不再把泛指的 `CUDA error` 当 OOM
  —— 避免为普通 CUDA 异常白重试 3 次（每次都要重新加载并量化 26GB 底模，用户会以为卡死）。

### 优化：采样预览默认开关与档位判定统一
- 阈值由 `16.5` 改为 `< 20`，与自身注释「20G+ 默认开」一致；
  `16.5~20GB` 的机器默认不再开启训练中采样（更省显存），用户仍可手动勾选。

### 测试
- `smoke_test.py` 新增 16.0x~19G「死区」回归断言（16.01 / 16.6 / 17 / 18.9 必须拿到 qint8 + 512 + layer_offloading）；
- 修正 `engine_install_smoke_test.py` 中已过期的 `"自动切换魔搭"` 断言 —— 它导致整个测试套件长期中断在此处，后续用例从未被执行。

### 影响面
- 经 A/B 逐字段实测（22 组显存值）：**≤16GB 与 ≥19.5GB 显卡的输出完全一致**，
  只有 16.01~19.4GB 这一段从「必 OOM」变为「能跑」。

## v0.16.4（2026-09-11）

### 优化：离线标签词典 / 标签编辑器输入卡顿
- **根因**：词典窗把 `<KeyRelease>` 直接接到查询，而结果区每次都「先销毁全部结果行、再新建数百个 CustomTkinter 控件」；
  中文输入法组合期会连发多次按键 → 每次输入都在重建约 360 个控件，实测单次 **≈0.73s**。
- **修复**：
  - 按键**防抖 150ms**（回车 / 筛选 / 排序即时）；输入未变化不再重复刷新；
  - 结果行改为**控件池复用**（预建 40 行，之后只改文本与显隐）→ 实测单次刷新 **≈0.02s**；
  - 单次结果上限 60 → 40 条；窗口关闭后不再触发延迟查询（守卫）。

### 优化：首次打开词典不再卡顿
- 词库载入（17.5 万条，≈0.20s）与中文索引（≈0.16s）原本在主线程同步执行；
- 现改为**启动后后台线程预热**，并统一为**进程级共享单例**（标签编辑器 / 独立词典窗 / 预热线程共用一份，只解析一次）；
- 载入加锁串行化，并修正中文索引的写入顺序，避免预热线程与界面线程并发时读到半成品。

### 优化：编辑器补全不再重算标签频率
- 标签编辑器的英文补全原先每次输入都重扫整个数据集统计标签频率；现按数据集版本号缓存（刷新 / 保存 / 插入时自动失效）。

### 界面：🧰 小工具版式整理
- ⑦ 离线标签词典补充状态行（与其余卡片等高）；
- ⑥ 下载源去掉重复说明文案（原「标题提示 / 复选框 / 状态行」三处重复 → 各司其职），复选框简化为「官方源优先」，状态行随开关实时更新；
- 卡片与状态行间距收紧，7 张卡片 + 底部「详细输出」一屏放下。

### 其他
- 无功能行为变化；训练、预处理、打标链路未改动。

## v0.16.3（2026-09-11）

### 新增：主页「🧰 小工具」→ ⑦ 离线标签词典（独立入口）
- 内置 **175,336 条**中英标签（纯离线，7MB）；以前只能在标签编辑器里打开，现在主页小工具窗直接进；
- 独立模式动作为**复制到剪贴板**：双击条目或点「复制」复制英文标签；右键菜单可**复制中英对照**或只复制中文；
- 新增**标签篮**：结果行点「+」收集，顶部「🧺 已收集 N」弹小窗管理（全部复制成逗号分隔一行 / 单条移除 / 清空），写长提示词时可"边查边攒"；仅内存、关软件即清空；
- 新增**分类筛选 + 排序**：按 通用 / 角色 / 作品 / 画师 / 元数据 过滤；排序可选 默认顺序 / 按热度 / 按字母（两个入口都有）；
- 标签编辑器里的词典窗行为**不变**（仍是「插入当前图标签」「翻译当前图标签」），只是多了筛选与排序。

### 界面重做：「🧰 小工具」窗口
- 每个功能改为**独立卡片**（卡片间距即视觉分隔），不再每个功能配一个大日志框；
- 运行结果改为卡片内**一行状态提示**（与普通工具一致），例如「✅ 已清理 12 个进程 · 释放约 1.37 GB」「✅ 已清理 2 项 · 释放 246 MB」；
- 完整输出统一收进底部「**详细输出**」（默认折叠，点开才展开）——需要排查时随时可看，且仍照常写入主日志（反馈日志不受影响）；
- 窗口 840×820 → **780×700**，一屏可看全 7 个功能。

### 其他
- 两个词典窗各自独立、互不影响；词库仍是进程级单例（只解析一次）。

## v0.16.2（2026-09-11）热修复

### 🔴 紧急修复：新版点「创建项目」没反应（0.16.1 引入）
- **根因**：0.16.1 新增的「自动清洗概念标签」开关变量建在了**延迟加载的主卡片构建函数**（`_build_main_cards`）里；
  而主页点「创建项目」会在主卡片构建**之前**调用 `_collect_project_data()` → `_collect_params()`，读不到该变量直接 `AttributeError`；
  异常被 Tkinter 吞掉，界面表现就是「点了创建没反应、对话框不动」。
- **修复**：把 `concept_type_var` / `clean_concept_var` 移到 `__init__`（与其他持久变量一致），
  并新增 `_clean_concept_flag()` 防御取值（主卡片未构建时默认 True）。
- **影响面**：**0.16.1 的所有用户、所有模式**，点「创建/新建项目」都会失败 → 请更新到 0.16.2。
- 同类隐患已复查：`_update_mode_ui` 里的行控件访问一律走 `getattr` 守卫；`fast_tier_var` 原有守卫保留。

### 其余
- 功能与 0.16.1 完全相同（概念类型 / 概念标签自动清洗 / 强绑定只锁 trigger）。

## v0.16.1（2026-09-11）

### 修复：概念模式「服装 / 物品」类 LoRA 单独用触发词出不来
- **根因**：WD14 会把服装拆成一堆通用标签（black jacket / long sleeves / collared / buttons / zipper），
  基础模型靠这些标签本身就能画出衣服 → trigger 被架空；强绑定又把这些 100% 一致的标签一起锁进固定前缀，
  等于每一步都在告诉模型「这件衣服靠这些词描述」，所以只写 trigger 出不来。
- **修复**：概念模式新增「**自动清洗概念标签**」
  - 👗 服装 / 🧜 形态：按词表精准删除描述概念本身的标签（服装词表覆盖 collar/button/zipper/sleeve/pleated 等变体）；
  - 🗿 物品 / 👁 身体部位：词表无法穷举，改删「训练集里 **100% 一致**」的标签
    （已排除姿势 / 场景 / 视角词，避免把背景一起删掉）；
  - 清洗后日志会列出**具体删了哪些标签**，方便核对；误删可在界面关闭该开关。
- **配套**：概念模式下强绑定改为「**只锁 trigger**」（keep_tokens = trigger 词数），
  不再把 100% 一致的标签一起锁进前缀；这些标签改为给「一致性警告」，提示加多样性——
  避免背景/姿势被一起绑进 trigger。
- **UI**：概念类型下方新增「自动清洗概念标签（推荐）」开关（默认开）；随项目配置保存/恢复。

### 修复：形态词表尾部空分支导致标签被删光
- `form` 概念词表结尾多了一个 `|`，会让正则匹配空字符串、把所有标签都删掉；已修正。

## v0.16.0（2026-09-10）

### 新增：概念模式支持「概念类型」（形态 / 服装 / 物品 / 身体部位）
- 概念模式新增「概念类型」下拉；它只影响 **数据集提示 + 采样预览提示词**，训练逻辑不变（仍是「训练集里唯一一致的东西绑定到 trigger」）；
- 数据集提示随类型变化：服装/物品明确提示「换不同人脸、不同画风、不同姿势、不同背景，只让服装/物品保持一致」；
- 采样预览提示词随类型变化：
  - 🧜 形态/种族、👗 服装 → `trigger, 1girl, solo, full body, ...`（按训练集自动补主体）；
  - 👁 身体部位 → `trigger, 1girl, solo, close-up, ...`；
  - 🗿 物品 → `trigger, ...`（**不补主体**，避免出「人拿着物品」或飘着的物件）；
  修复了概念模式此前一律不加主体词、练服装时采样预览出「飘着的衣服 / 畸形人物」的问题；
- 图像模型（Qwen-Image / Z-Image / Krea2 / FLUX.2）的「概念」子模式同样支持概念类型。

### 修复：概念模式「强绑定」守卫不一致
- 此前概念模式在**预处理阶段**会做强绑定（trigger + 训练集 100% 一致特征 → 固定前缀 + keep_tokens），但**训练入口**的 7 处调用点都被 `mode == "character"` 挡掉；
- 结果：预处理之后改 trigger 时，人物模式会重算前缀，概念模式不会（前缀里可能残留旧 trigger）；
- 已统一为与人物模式一致（预处理 + 训练入口都执行，幂等）。

### 其他
- 项目配置 / 导出分享配置新增 `concept_type` 字段；旧配置缺失时自动回退「形态/种族」，不影响既有项目。
...
- 根因：完成判定一刀切——"存在最终 .safetensors 就不提示续训"。若项目以前跑完过、这次又中断，旧成品
  ...
  中间 step/epoch 模型文件误当成品。
- 已修：只比较【最终 {名}.safetensors】与【最新断点目录】的修改时间——成品比断点新/等=已跑完不提示；
  成品比断点旧（旧完成+新中断）=仍提示续训；不再把中间模型文件算进判定。
...
- Krea2AT ≤20G 不再量化文本编码器：DiT qint8≈13GB 已占满 16G，TE 量化是初始化峰值（5070 Ti 实测 OOM）；
  TE 走 CPU+bf16、缓存 embedding 后卸载；≥20G 维持量化；
- Krea2AT 关采样也保留 sample 段 + disable_sampling：否则引擎 cache_sample_prompts 用默认 bool negative_prompt 崩溃；
- 快跑档=开 只做"降分辨率 + 关采样"，量化/层交换按真实显存走对应档（不再把显存模拟成 8G，避免过度层交换又慢又 OOM）。

## v0.15.7（2026-09-08）

### 新增：训练队列 v1（多选项目 → 逐项预处理+训练）
- 主页「⏳ 训练队列」：左侧多选已保存项目加入队列、右侧移除/清空，▶开始 / ⏹停止；
- 逐项执行「预处理 → 训练」，可跨第 1~4 引擎；失败项保留在队列可重跑；停止 = 停当前项并中止整队；
- 仅运行期有效；H3 视频暂不入队；项目保存/恢复补 style_caption（画风模式入队不丢描述词）。

### 新增：标签编辑器多选操作
- 图片列表支持 Ctrl/Shift 多选，「删除选中图片」一次删多张（含同名 txt/npz）；
- 标签统计窗支持多选，批量删除选中标签（从全部图片移除）。

### 修复：训练队列窗口字体（ui_font 包装，点「训练队列」不再报错）

## v0.15.6（2026-09-07）

### 调整：采样预览与「中间保存快照」同节奏 + 间隔可自选
- kohya（第1引擎）：采样预览由写死每 100 步改为跟随保存间隔（默认 200 步）；
- Krea2 / FLUX.2（musubi）：保存按轮，采样换算成「每轮步数 × 保存轮数」与快照同步；
- 高级参数新增「采样预览间隔(步)」：0/留空 = 跟随保存快照（默认），填 N = 固定每 N 步出预览；随项目保存/恢复。
...
### 新增：Z-Image 8G 快跑档（≤8G 显存也能稳定训练，根治卡死）
- 8G 显存用户日志实证：旧配置 512/qfloat8/low_vram 模型可加载，但卡在训练前 baseline 采样；
  纯 qfloat8 时 fp8 DiT 全量驻留显存 ~7.8G、无激活余量，训练起不来。
- 快跑档（≤8G 自动生效）：
  · 关闭训练采样（官方 disable_sampling）——不再卡 baseline / 每 250 步生图；
  · 分辨率按内存自动钳制：内存 <32G→384（实测稳定）、≥32G→512；
  · 文本编码器量化（quantize_te qfloat8）+ 官方 timestep_type=weighted；
  · 开启官方 layer_offloading 层交换 0.6（引擎自动转 float8 逐层换入）。
- Z-Image / Qwen-Image 的「分辨率」改为界面可调（原被隐藏写死 1024）。
- 真机实测（RTX 4070 Laptop 8G + 16G 内存，384 + 层交换）：~9–12s/it 稳定、loss 正常、无 OOM。

### 新增：快跑档手动开关（自动 / 开 / 关）
- Z-Image 模式界面新增「⚡ 快跑档」下拉：自动=仅 ≤8G 触发；开=强制启用（12G/16G 想提速/求稳可用）；关=完全按常规。
- 档位随项目保存/恢复；训练确认弹窗显示当前档位；smoke 测试覆盖三种档位。
...
- 画风模式没填「画风描述词」时应逐张 WD14/内置打标；老版本曾把固定的兜底描述（anime cel-shading…）整批写死，
  新版已在重跑时自愈清除并重新打标。
- 再加一层自检：预处理结束时若画风标签整批高度一致（<=2 种）会显眼告警，提示自动打标未生效，避免静默拿到"整批同句"去训练。

### 新增：Anima 合并包底模自动识别 + 一键剥离 DiT 缓存
- 社区发布的 Anima「Semi/合并」推理包常把 Qwen3 文本编码器（cond_stage_model.*）一起塞进底模，
  第一引擎 anima 训练要求纯 DiT，直接用会报 Unexpected keys in checkpoint 退出（2026-09-05 用户日志实证）。
- 新增 kohya_core/anima_ckpt.py：只读 safetensors 头判断 pure/merged；流式剥离 DiT 权重（不整包加载进内存），
  有同目录纯 DiT 参考时按 key 交集过滤保证"零多余 key"；剥离结果缓存到数据目录，二次直接复用。
- 界面：训练/一键训练前检测到合并包会弹提示，确认后自动剥离并缓存再训练（大文件首次 1~3 分钟）；
  训练内核入口同步兜底自动剥离，命令行/排队等场景同样生效。
...
- 已修：下载前 Range 探测源是否支持续传，不支持则删残片整包重下；有 .part 续传时自动把魔搭排到支持 Range 的上海交大/阿里云之后；curl 加 --fail（404/5xx 不再把错误页当"下载成功"）；pip 判为损坏的缓存轮子自动清除、下次整包重下（纯网络失败仍保留缓存）。

### 修复：Krea2 训练中采样预览（第二/三/四引擎，之前输出文件夹里都没有预览图）
- 第二引擎（musubi）：≤16.5G 采样分辨率自动压到 512（原按 768/1024 采样易 OOM，musubi 采样无容错会中断训练）；
- 第三引擎（ai-toolkit）：不再 ≤16G 一律硬关，改为跟随勾选框（16G 默认关、勾选才开；开时采样压 512 + 提示）；
- 第四引擎（Fizgig）：接入采样预览——需 models/krea2/turbo.safetensors（fp8 Turbo 约 13GB，软件内可下载），按 epoch 折算约每 100 步出图，16G 自动 512 + preview_int8；缺 turbo 只提示不中断训练。

### 优化：第四引擎 AMD ROCm 档位
- auto 档 ≥18G 改 int8 + 关块交换（7900 XT 实测：auto 的 fp8+块交换在 ROCm 上很慢，int8+0 最快）；
- AMD 优化器回归 Fizgig 官方默认（不再强制 adamw）。

### 优化：训练残留进程自清理 + 工具页清理支持 AMD
- Krea2 / FLUX.2 / Fizgig 训练前自动清理残留训练进程（防残留进程占满内存，导致新训练卡在加载 26GB 底模十几分钟）；
- 工具页「清理显存」→「清理残留训练进程」，N 卡 + AMD 通用（WMI 命令行匹配，无 nvidia-smi 也能用）。

### 其它
- ai-toolkit 采样 yaml 补 negative_prompt 空串；
- Krea2/Fizgig 低显存 fp8+块交换"显存占用偏低属正常、训练中请勿关闭"说明。
...
### 回滚：v0.14.1 的「升级后自动重启」改动（VBS 助手在部分环境异常）
- 反馈：仍不自动重启，且部分环境手动打开报错（VBS 助手/安装器时序问题）。
...
- **背景**：4080S 用户实测：Krea2 768px 训练第 2~100 步稳定 ~13.9s/it，**第 100 步（训练中采样出图）后一路退化到 54s/it**，696 步要 7 小时+。
- **根因**：每 100 步采样要加载 Qwen3-VL 文本编码器（8GB）出图，采样后显存残留 → 16G 卡（模型本身 ~13GB 占满）后续训练换页、越来越慢。采样预览此前对所有人默认开。
- **已修**：16G 档（显存 ≤16.5G）「训练中采样预览」**默认关**（20G+ 不受影响照常开）；16G 卡若手动勾选开启，训练时打印醒目警告。
- **验证**：新增 SAMPLE_PREVIEW_16G_DEFAULT_OFF_OK，engine_install_smoke_test + smoke_test 全过。

### 新增：高级参数「torch.compile 加速」勾选（Krea2/FLUX.2）
- musubi 原生 `--compile` 编译 28 个块提速（实测 fp8+compile ≈2×，int8+compile ~8%）；Windows 存在编译兼容风险，故做成默认关的手动勾选，随项目保存。

## v0.11.0（2026-08-28）

### 界面重做（方案A）：侧边栏「引擎导航」，三个引擎清晰分明
- **背景**：第一/第二/第三引擎之前藏在顶部「训练模式」下拉里，不分明；功能堆叠在顶部左侧，后续加引擎/加功能都麻烦。
- **已做**：左侧新增**引擎导航**——按引擎分组（第一引擎·kohya / 第二引擎·musubi / 第三引擎·ai-toolkit），组内用**模式按钮**（画风 / 人物 / Krea2 / FLUX.2 / 视频H3 / Qwen / Z-Image）直接切换，当前模式高亮；顶部「训练模式」下拉移除（模式入口统一到侧边栏）；新手引导继续按引擎/模式动态联动。
- **扩展性**：以后加新引擎 = 侧边栏加一组 + 注册表（MODE_KEYS / PRESETS）加一条，主区卡片复用。

### 修复：模式切换后 UI 卡死（style+flux2 预设查表 KeyError）
- **根因**：`PRESETS["style"]` 没有 `flux2` 档；画风模式选 FLUX.2 底模后切换模式，`_apply_presets` 抛 KeyError → 界面刷新中断，UI 不再变化。
- **已修**：预设查表改防御式（`preset_for` 回退该模式默认档）+ 切换模式时自动把不兼容的底模类型重置回默认（如画风模式下的 FLUX.2 底模自动回 SD1.5）。

### 修复：Z-Image / Qwen-Image 魔搭下载报 WinError 183
- 魔搭 repo/files API 的**目录项**（Type=tree）被误当文件下载，写到已存在的同名目录时报 `[WinError 183] 当文件已存在时，无法创建该文件` → 预下载中断 → 回退在线加载（hf-mirror 不可达）→ 训练失败。现只下载文件（Type=blob），跳过目录项。
...
- **背景**：4080S 16G 用户实测链路——swap=6 时显存 15.5/16.0GB 顶满 → Windows 换页到内存：v0.10.17 卡死一下午（第一步 0/928 不动）、v0.10.18（fp8）64→83→176s/it 越跑越慢；swap=12 时则每步搬 12 块、PCIe 瓶颈 11~17s/it。两头堵。
- **根因（第一性原理）**：16GB 装不下 12.9B 的 Krea2 底模（fp8/int8 均约 12.9GB）+ 激活 + 优化器 + 显存开销。驻留块多 → 换页（比搬运慢 10~50 倍）；驻留块少 → 搬运慢。社区 16G 快（5060 Ti 3~4s/it）靠的是 4~5bit 量化或 pinned memory 加速搬运（OneTrainer PR #1630 / musubi 官方 #782）。
- **已修**：
  - 16G 档 blocks_to_swap 6→10（驻留 22→18 块，显存 ~13GB 留足余量，杜绝换页）；
  - 16G+ 自动开 `--use_pinned_memory_for_block_swap`（musubi 官方块交换搬运加速；8G 仍关闭避免 OOM，2026-08-22 本机验证）；
  - 高级参数新增「块交换数」下拉（自动 / 2 / 4 / 6 / 8 / 10 / 12），跑稳后可手动往下调换速度，随项目保存；
  - 量化保持 fp8（社区 16G 主流），int8 只留 8~12G。
- **验证**：SWAP_TIER_RESOLUTION 更新（15.67→(10,True) / 18→(10,False)），engine_install_smoke_test + smoke_test 全过。

## v0.10.18（2026-08-27）

### 修复：Krea2/FLUX.2 16G 卡训练仍慢——16G 档默认量化照搬社区改 fp8，新增「量化方式」可选
- **背景**：v0.10.17 修好「16G 误判 12G 档」后（swap 12→6），4080S 用户实测：GPU 利用率 100%、功耗仅 ~80W、显存带宽占用仅 5%——搬运瓶颈已消失，但卡在 **int8（W8A8）算子低效**（100% 利用率却只有 1/4 功耗，典型低效算子特征）。
- **根因**：工具 16G 档默认 int8，源自 8G 卡 + 每步大块交换场景的实测（int8 省带宽、快 2.6×）；但 16G 卡带宽不是瓶颈（PCIe gen4 x16 + GPU 满负荷），int8 的省带宽优势失效，只剩算子低效。社区 16G 主流配置 = **fp8_scaled + 少量块交换**（musubi 官方推荐路径，Tensor Core 效率高；SECourses 的 int8 更快是其自编译内核，无法照搬）。
- **已修**：
  - Krea2/FLUX.2 显存档位取整后 **16G+ 自动默认 fp8_scaled**（社区主流），int8 只留给 8~12G 带宽瓶颈档；
  - 高级参数新增「量化方式（Krea2/FLUX.2）」下拉：自动 / fp8 / int8 / nf4，可自行对比，选择随项目保存；
  - 日志档位文案同步更新（16G+/自动档 fp8）。
- **验证**：QUANT_MODE_RESOLUTION 新增 (15.67→fp8)/(16→fp8)/(12→int8) 断言，engine_install_smoke_test + smoke_test 全过。
