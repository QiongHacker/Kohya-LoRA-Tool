...
def _resolve_krea2_swap(vram_gb, gc_on=True, ram_gb=None):
    ...
    if tier is None or tier < 12:
        ...
        swap = 12          # 16G 档：5070 Ti 实测「int8+swap12+pinned+512px」= 2.5s/it（2026-09-14 用户反馈）
                           # 早期 4080S 记录 7s/it（未标分辨率，疑似 768+，勿与 512 直接对比）
...
#
# 覆盖两类残缺（Windows ROCm 版 torch 常见）：
#   1) torch.distributed 缺 is_initialized / destroy_process_group 等
#      → 训练收尾时 accelerate 崩、最终模型不保存；
...
#        抛 ModuleNotFoundError: No module named 'torch._C._distributed_c10d'; 'torch._C' is not a package
#        训练一步都跑不起来（2026-09-15 RX 9070 XT + ROCm 7.2.1 反馈）。
import sys as _sys
...
if any(_k in _argv for _k in ("train_network", "sdxl_train", "anima_train", "flux_train",
    ...
    # DTensor 占位：真模块 import 不了时用空壳顶上，让 accelerate.utils.other.model_has_dtensor()
    # 的 import 成功，且 isinstance(p, DTensor) 恒为 False（单卡训练本来就没有 DTensor）。
    try:
        ...
        # 清掉 import 失败留下的半成品，避免后续拿到损坏模块
        for _k in [k for k in list(_sys.modules) if k.startswith("torch.distributed.tensor")]:
            del _sys.modules[_k]
        ...
        class _StubModule(_types.ModuleType):
            def __getattr__(self, _name):
                ...
                # 任何按名取用的符号（DTensor / Shard / Replicate…）都返回独立占位类
                _cls = type(_name, (), {})
        ...
        class _StubLoader:
            def create_module(self, spec):
                _mod = _StubModule(spec.name)
                _mod.__path__ = []          # 看起来像包，允许 torch.distributed.tensor.xxx
                return _mod
        ...
        class _StubFinder:
            _PREFIX = "torch.distributed.tensor"

            def find_spec(self, fullname, path=None, target=None):
                if fullname == self._PREFIX or fullname.startswith(self._PREFIX + "."):
                    return _ModuleSpec(fullname, _StubLoader())
                return None

        _sys.meta_path.insert(0, _StubFinder())
        import torch.distributed.tensor as _stub  # noqa: F401
'''
...
def write_at_image_yaml(params, info, train_dir, out_dir, cfg_path, vpy=None, logf=print, vram_gb=None):
    ...
    text = (
        ...
        "        gradient_checkpointing: true\n"
        "        noise_scheduler: \"flowmatch\"\n"
        "        timestep_type: " + ("'weighted'" if _fast8_tier else "'linear'") + "\n"
        "        optimizer: " + _yq(_opt_yaml) + "\n"
        "        lr: " + repr(lr) + "\n"
        "        optimizer_params:\n"
        "          weight_decay: 1e-4\n"
...
def _resolve_quant_mode(mvpy, logf, vram_gb, label="Krea2", requested="auto", allow_nf4=True, prequantized=False):
    ...
    # 结论：int8 的 2.6× 优势在 8G 大块交换（带宽瓶颈）场景；16G：
    #   5070 Ti 实测 int8+swap12+pinned+512px = 2.5s/it（2026-09-14 用户反馈，最新基准）；
    #   4080S 早期记录 int8+swap12+pinned = 7s/it、fp8+swap10+pinned = 30~40s/it（2026-08-27，
