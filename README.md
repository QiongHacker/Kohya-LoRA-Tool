...
| Krea2（F）图像 LoRA | Krea2（Fizgig 引擎） | 第四引擎（Fizgig，NVIDIA / AMD ROCm 双平台） |

主要能力：

- **底模自动识别**：选择模型后自动判断 SD1.5、SDXL、FLUX、Anima 等架构，并切换合适的分辨率与参数预设。
- **画风 / 人物 / 概念模式**：画风模式过滤人物干扰标签；人物模式保留角色特征并支持 Trigger；概念模式适合训练“美人鱼、半人马、木偶”这类难以用词形容的形态 / 种族。
...
- **内置 WD14 打标模型**：开箱即用，优先 GPU；官方打标脚本缺失或环境损坏时自动改用内置打标（不依赖第一引擎），并自动补装依赖、清理上次失败的兜底标签。
...
- **标签编辑器**：逐图查看、批量删除/替换、Trigger 置顶、标签频率统计；内置**离线中英词典**（中英互译 / 自动补全 / 整段翻译 / 一键插入当前图）。
...
- 国内网络优先使用魔搭、中科大、华为云、清华和 Hugging Face 镜像，并支持大文件断点续传与自动重试。
...
- 各引擎的 PyTorch 大轮子统一使用上海交大/魔搭等国内镜像断点续传，避免无代理用户被迫访问海外源；下载前会先实测各源吞吐并自动择优（阿里云实测偏慢，已降为兜底）。
- NVIDIA 为主要支持平台；AMD 用户训练 Krea2 图像 LoRA 推荐走第四引擎（Fizgig ROCm 官方支持），其余引擎提供实验性兼容模式与环境检查提示。
...
| `kohya_core/tagging/` | 标签管理中英词典 / 翻译 / 补全核心（离线词条，纯 Python 可单测） |
| `gui/tag_tools.py` | 标签编辑器配套工具（中英词典窗等） |
| `installers/` | Git、Python、训练引擎等离线安装资源 + 内置离线中英词典（tag_dict） |
